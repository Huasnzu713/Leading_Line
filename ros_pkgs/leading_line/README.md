...
然后把项目根的 `jetson/` 和 `protocol/` 软链到 `~/ros_ws/src/leading_line/` 下，
让 `leading_line_node.py` 能 import 算法：
...
- xcar 协议：[`scripts/xcar/xcar_data.py`](scripts/xcar/xcar_data.py)（串口帧解析）