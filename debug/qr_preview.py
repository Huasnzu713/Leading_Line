...
def resolve_config(arg_path: str) -> str:
    """--config 走用户路径；未传则用 config.yaml 的 debug.qr_preview 段。"""
    if arg_path and arg_path != DEFAULT_CONFIG:
        return arg_path
    if not Path(DEFAULT_CONFIG).exists():
        cwd_fallback = Path("config.yaml")
        if cwd_fallback.exists():
            return str(cwd_fallback)
    return DEFAULT_CONFIG


def _qr_preview_cfg(cfg: dict) -> dict:
    """取出 config.yaml 里的 debug.qr_preview 段（容错：找不到给空 dict）。"""
    return (cfg or {}).get("debug", {}).get("qr_preview", {}) or {}


# ---------- 模式 1：test ----------
def run_test(source: str, cfg: dict) -> int:
    """离线跑一张图，识别一次，喂给状态机，把结果存图。"""
    qcfg = _qr_preview_cfg(cfg)
    p = Path(source)
    if not p.exists():
    ...
    sm = QRStateMachine(
        policy_timeout_s=float(qcfg.get("policy_timeout_s", 30.0))
    )
    sm.start()
    ...
    for _ in range(int(qcfg.get("test_ticks", 3))):
    ...
    out_dir = _PROJECT_ROOT / "qr_result"
...
def run_camera(source: Optional[str], cfg: dict) -> int:
    ...
    qcfg = _qr_preview_cfg(cfg)
    cam_cfg = qcfg.get("camera", {})
    window_name = str(qcfg.get("ui", {}).get("window_name", "QR State Machine"))
    exit_key = int(qcfg.get("ui", {}).get("exit_key", 27))
    save_key = ord(str(qcfg.get("ui", {}).get("save_key", "s")).lower() or "s")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="QR + 状态机策略系统（调试）")
    ...
    p.add_argument("--config", default=DEFAULT_CONFIG,
                   help=f"YAML 配置路径；默认用 config.yaml 的 debug.qr_preview 段")
    return p.parse_args()
