...
1. catkin 工作空间里 `src/` 下放好 `mbot/`（zonesion xcar 源码）、
   `realsense2_camera/`（realsense 驱动）、`leading_line/`（本包）
2. 把项目根的 `jetson/` 和 `protocol/` 软链到 `src/leading_line/` 下
3. `catkin_make && source devel/setup.bash`
4. `roslaunch leading_line leading_line_with_car.launch`

启动后 `mbot/launch/serial-node-zonesion.launch` 内部启动 `xcar_ros.py`，
自动订阅 `/cmd_vel` 并通过 `/dev/ttyXCar` 串口发到 xcar MCU。
