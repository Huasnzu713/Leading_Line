# -*- coding: utf-8 -*-
"""pipeline.py — headless 主流水线（cv2 摄像头 + 算法 + override + UDP 推流 + TCP 命令）。

无 ROS 依赖，运动控制通过注入的 `publish_cmd` 回调发布；上游可传入
`leading_line.ackermann_bridge` 的 rclpy/单测实现，或调试桩。

历史：原本在 vehicle/pipeline.py；ROS2 重构后迁入本包，作为单一来源。
"""
...
from leading_line.overrides import (
    FrameOverrides,
    SRC_ARROW,
    SRC_QR,
    SRC_PATH,
)
from leading_line.recognition.arrow_detector import annotate as arrow_annotate
from leading_line.recognition.qr.decoder import draw_qr_overlay
...
# publish_cmd 签名：上游（ROS2 ackermann_bridge、UDP 调试桩、单测等）注入
# 入参 (steer_deg, speed)，speed 是 0..1 无量纲；上限速度由调用方自己乘
PublishCmd = Callable[[float, float], None]
...
class Pipeline:
    ...
    运动控制发布是**注入**的（`publish_cmd` 回调），不再 import 任何 ROS 桥。
    上游可传入 rclpy ackermann_bridge.publish、UDP 桩、单测 MockBackend 等。
    """
    ...
    def _stop_motion(self) -> None:
        ...
        try:
            ...
            log.debug("publish_cmd(stop) 异常: %s", e)
    ...
    def _process_frame(self, frame):
        ...
        if self.rt.state == STATE_RUNNING:
            try:
                self.publish_cmd(final_steer, final_speed)
            except Exception as e:  # noqa: BLE001
                log.debug("publish_cmd 异常: %s", e)
            ...
            while not self._stop:
                ...
                if now - self.rt.last_fps_t >= 1.0:
                    ...
                    log.debug(
                        ...
                        self.backend_name, self.rt.last_source, self.rt.last_qr_state,
                    )
