...
```
 ...
 FrameOverrides.tick(frame, dt)         ← 本模块
     │
     ├── qr_state_machine  → (steer_qr,  speed_qr)  优先级最高
...
class FrameOverrides:
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self.qr_policy_timeout_s: float = float(qr_cfg.get("policy_timeout_s", 30.0))
        self.qr_on_state_change: Optional[callable] = None  # 主循环注入：状态变化时打 INFO

        self._sm: Optional[QRStateMachine] = None
        if self.qr_enabled:
            self._sm = QRStateMachine(policy_timeout_s=self.qr_policy_timeout_s)
